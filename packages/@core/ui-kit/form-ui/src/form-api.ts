...
import type { FormActions, FormSchema, SuperFormProps } from './types';

import { isRef, toRaw } from 'vue';

import { Store } from '@super-core/shared/store';
import {
  bindMethods,
  cloneDeep,
  createMerge,
  ...
  set,
  StateHandler,
} from '@super-core/shared/utils';

import { resolveFieldNamePath } from './field-name';

function getDefaultState(): SuperFormProps {
  return {
    actionWrapperClass: '',
    collapsed: false,
    collapsedRows: 1,
...
export class FormApi {
  // private api: Pick<SuperFormProps, 'handleReset' | 'handleSubmit'>;
  public form = {} as FormActions;
  isMounted = false;

  public state: null | SuperFormProps = null;
  stateHandler: StateHandler;

  public store: Store<SuperFormProps>;
  ...
  private prevState: null | SuperFormProps = null;

  constructor(options: SuperFormProps = {}) {
    const { ...storeState } = options;

    const defaultState = getDefaultState();

    this.store = new Store<SuperFormProps>({
      ...defaultState,
      ...storeState,
    });
  ...
  setState(
    stateOrFn:
      | ((prev: SuperFormProps) => Partial<SuperFormProps>)
      | Partial<SuperFormProps>,
  ) {
  ...
  private async getForm() {
    ...
    if (!this.form?.meta) {
      throw new Error('<SuperForm /> is not mounted');
    }
    return this.form;
  }
