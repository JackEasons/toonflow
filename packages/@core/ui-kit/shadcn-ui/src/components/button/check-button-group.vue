...
import type { ValueType, SuperButtonGroupProps } from './button';

import { computed, ref, watch } from 'vue';

import { Circle, CircleCheckBig, LoaderCircle } from '@super-core/icons';
import { cn, isFunction } from '@super-core/shared/utils';

import { objectOmit } from '@vueuse/core';

import { SuperRenderContent } from '../render-content';
import SuperButtonGroup from './button-group.vue';
import Button from './button.vue';

const props = withDefaults(defineProps<SuperButtonGroupProps>(), {
  gap: 0,
  multiple: false,
  showIcon: true,
  size: 'middle',
...
<template>
  <SuperButtonGroup
    :size="props.size"
    :gap="props.gap"
    class="super-check-button-group"
  >
    <Button
    ...
    >
      ...
      <slot name="option" :label="btn.label" :value="btn.value" :data="btn">
        <SuperRenderContent :content="btn.label" />
      </slot>
    </Button>
  </SuperButtonGroup>
</template>
<style lang="scss" scoped>
.super-check-button-group {
  display: flex;
  flex-wrap: wrap;
