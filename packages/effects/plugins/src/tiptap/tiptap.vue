...
import type {
  ...
  ToolbarAction,
  ToolbarMenuItem,
  SuperTiptapChangeEvent,
} from './types';

import { computed, onBeforeUnmount, reactive, watch } from 'vue';

import { Check, ChevronDown, Eye } from '@super/icons';
import { $t } from '@super/locales';

import { useSuperModal } from '@super-core/popup-ui';
import { SuperIconButton, SuperPopover } from '@super-core/shadcn-ui';
...
const emit = defineEmits<{
  change: [payload: SuperTiptapChangeEvent];
}>();
...
const tiptapContentClass = cn(
  'super-tiptap-content super-tiptap__content',
  'text-foreground max-h-(--super-tiptap-max-height) min-h-(--super-tiptap-min-height) overflow-auto leading-7 outline-none',
);
...
const [PreviewModal, previewModalApi] = useSuperModal({
  footer: false,
...
<template>
  <div
    :style="{
      '--super-tiptap-min-height': contentMinHeight,
      '--super-tiptap-max-height': contentMaxHeight,
    }"
    class="super-tiptap overflow-hidden rounded-xl border border-border bg-card"
  >
    <div
      v-if="toolbar"
...
<style scoped>
.super-tiptap
  :deep(.super-tiptap__content p.is-editor-empty:first-child::before) {
  float: left;
  height: 0;
