...
import type { AnyFunction } from '@super/types';

import { computed, useTemplateRef, watch } from 'vue';

import { useHoverToggle } from '@super/hooks';
import { LockKeyhole, LogOut, Settings } from '@super/icons';
...
import { isWindowsOs } from '@super/utils';
...
const [LockModal, lockModalApi] = useSuperModal({
  connectedComponent: LockScreenModal,
});
const [LogoutModal, logoutModalApi] = useSuperModal({
  onConfirm() {
    handleSubmitLogout();
  },
});
    ...
    <DropdownMenuTrigger ref="refTrigger" :disabled="props.trigger === 'hover'">
      <div class="mr-2 ml-1 cursor-pointer rounded-full p-1.5 hover:bg-accent">
        <div class="flex-center hover:text-accent-foreground">
          <SuperAvatar :alt="text" :src="avatar" class="size-8" dot />
        </div>
      </div>
    </DropdownMenuTrigger>
    <DropdownMenuContent class="mr-2 min-w-60 p-0 pb-1">
      <div ref="refContent">
        ...
        <DropdownMenuItem
          ...
          @click="menu.handler"
        >
          <SuperIcon :icon="menu.icon" class="mr-2 size-4" />
          {{ menu.text }}
        </DropdownMenuItem>
        <DropdownMenuSeparator />
        <DropdownMenuItem
