...
import { SuperRenderContent } from '@super-core/shadcn-ui';
...
declare global {
  const __SUPER_ADMIN_METADATA__: {
    authorEmail: string;
    authorName: string;
    authorUrl: string;
    buildTime: string;
...
const renderLink = (href: string, text: string) =>
  h(
    'a',
    { href, target: '_blank', class: 'super-link' },
    { default: () => text },
  );
...
} = __SUPER_ADMIN_METADATA__ || {};

const superDescriptionItems: DescriptionItem[] = [
  {
    content: version,
    title: '版本号',
  },
  ...
  {
    content: renderLink(SUPER_DOC_URL, '点击查看'),
    title: '文档地址',
  },
  {
    content: renderLink(SUPER_PREVIEW_URL, '点击查看'),
    title: '预览地址',
  },
  {
    content: renderLink(SUPER_GITHUB_URL, '点击查看'),
    title: 'Github',
  },
  {
    content: h('div', [
  ...
  <Page :title="title">
    <template #description>
      <p class="mt-3 text-sm/6 text-foreground">
        <a :href="SUPER_GITHUB_URL" class="super-link" target="_blank">
          {{ name }}
        </a>
        {{ description }}
      </p>
    ...
    <div class="card-box p-5">
      ...
      <div class="mt-4">
        <dl class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          <template v-for="item in superDescriptionItems" :key="item.title">
            <div class="border-t border-border px-4 py-6 sm:col-span-1 sm:px-0">
              ...
              <dd class="mt-1 text-sm/6 text-foreground sm:mt-2">
                <SuperRenderContent :content="item.content" />
              </dd>
            </div>
          </template>
        </dl>
