<template>
  <div class="memoryConfig">
    ...
    <t-form :data="formData" labelAlign="top" labelWidth="180px" class="memoryForm">
      ...
      <div class="actionRow f frr">
        <t-button theme="primary" type="button" :loading="saving" @click="handleSave">{{ $t("settings.memory.saveConfig") }}</t-button>
        <t-button theme="danger" type="button" variant="outline" :loading="clearing" @click="handleClearMemory">{{ $t("settings.memory.clearMemory") }}</t-button>
        <t-button theme="warning" type="button" variant="outline" :loading="saving" @click="handleRestory">{{ $t("settings.memory.restoreDefault") }}</t-button>
      </div>
    </t-form>
  </div>
</template>
