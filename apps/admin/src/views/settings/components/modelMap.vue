...
async function delPrompt(row: PromptItem) {
  try {
    await axios.post("/setting/modelMap/deletePrompt", {
      path: row.path,
    });
    if (promptForm.value.path === row.path) {
      unselectPrompt();
    }
    ...
    window.$message.error(err?.message || $t("settings.memory.modelMap.bindingFailed"));
  }
}
