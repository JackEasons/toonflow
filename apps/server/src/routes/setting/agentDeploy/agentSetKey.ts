...
export default router.post(
  ...
  async (req, res) => {
    const { key } = req.body;
    const vendorConfigData = await u.db("o_vendorConfig").where("id", "dramastudio").first();
    ...
    await u
      .db("o_vendorConfig")
      .where("id", "dramastudio")
      .update({
        inputValues: JSON.stringify(inputValue),
      });
    try {
      const resText = await u.Ai.Text(`dramastudio:claude-haiku-4-5-20251001`).invoke({
        prompt: "1+1等于几？,请直接回答2，不要解释",
      });
      if (resText.text) {
        await u.db("o_agentDeploy").where("key", "scriptAgent").update({
          model: "claude-sonnet-4-6",
          modelName: "dramastudio:claude-sonnet-4-6",
          vendorId: "dramastudio",
        });
        ...
        await u.db("o_agentDeploy").where("key", "universalAi").update({
          ...
          modelName: "dramastudio:claude-haiku-4-5-20251001",
          vendorId: "dramastudio",
        });
