...
function getVendorVersion(vendorDir: string, id: string) {
  const code = fs.existsSync(path.join(vendorDir, `${id}.ts`)) ? fs.readFileSync(path.join(vendorDir, `${id}.ts`), "utf-8") : "";
  return code.match(/version\s*:\s*["']([^"']+)["']/)?.[1] ?? "0";
}

function writeVendorCode(vendorDir: string, id: string, tsCode: string) {
  fs.mkdirSync(vendorDir, { recursive: true });
  fs.writeFileSync(path.join(vendorDir, `${id}.ts`), tsCode);
}
...
export default async (knex: Knex): Promise<void> => {
  ...
  await knex("o_image").where("state", "已完成").whereNotNull("errorReason").update({
    errorReason: null,
  });
  ...
  const existingStorageProvider = oss.getStorageProvider();
