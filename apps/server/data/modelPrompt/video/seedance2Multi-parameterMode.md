...
- 必须包含负面约束：在提示词正文中追加「负面约束」段，用于规避低清、模糊、画面闪烁、人物变形、身份漂移、服装漂移、场景漂移、前后帧不连续、畸形肢体、错乱手指、多余手指、腿部或身体长出手指、多余手臂、手掌融合、重复人物、字幕、水印、logo、UI界面、标题叠字。若模型/接口另有独立负面参数，系统会自动拆分传递；本 Skill 仍需把规避意图写入可读提示词
...
- **最前面先定义参考映射**：先输出”参考定义”段，集中声明 `@参考N : 主体名字/场景名字/道具名字，简述`；带 audio 的资产在同一行尾追加「，参考音频为：@参考N+1」（音频不另起一行，但占用编号）
- **分镜正文必须复用图片参考编号**：角色、场景、道具在分镜正文第一次出现及关键动作处必须写成 `@参考N 名称`，例如 `@参考1 陈宇`、`场景：@参考2 室内环境`、`手中的 @参考3 手机`。禁止只写名称不写 `@参考N`
- **只引用图片参考锁定画面**：正文里用于画面锁定的 `@参考N` 只能指向图片参考（role / scene / prop / 分镜图），不能指向音频参考
...
> **注意**：`@参考{编号}` 必须先在"参考定义"段声明；分镜正文中角色、场景、道具必须继续使用对应图片参考编号，格式为 `@参考N 名称`，用于锁定身份、空间和道具形态。
>
> 若资产带 audio，则把该音频写在所属资产同一行尾部，格式为 `@参考K: {资产名字}，{简述}，参考音频为：@参考K+1`；音频本身**不另起一行**，但仍占用编号 K+1，下一条资产从 K+2 开始。音频参考只能用于「音色：@参考N」，禁止用于场景、角色或道具画面引用。
...
正文引用示例：
```
分镜1 5s: 时间：夜，场景：@参考2 室内环境，镜头：大特写，正面平视，固定镜头，@参考1 陈宇的面部被手机屏幕冷光照亮，眼睛紧绷睁大，手指悬在 @参考3 手机上方但未触碰屏幕。无台词。背景保持 @参考2 室内环境的布局和冷调光线。
```
...
分镜1 {N}s: 时间：{...}，场景：{@参考N 场景名字}，镜头：{...}，{@参考N 角色名字} {...}。{...}。
分镜2 {N}s: 时间：{...}，场景：{@参考N 场景名字}，镜头：{...}，{@参考N 角色名字} {...}，保持上一镜的人物身份、服装、空间布局和光线方向。{...}。
...
```
...
8. **单分镜时长最低 1s**
9. **负面约束必填**：在完整提示词末尾追加「负面约束：低清、模糊、画面闪烁、人物变形、身份漂移、前后帧不连续、畸形肢体、错乱手指、重复人物、字幕、水印、logo、UI界面、标题叠字」；不得把负面约束写成解释或分析
10. **时长单位**：直接使用 videoDesc 中的秒数，格式为 `{N}s`（如 `4s`），最低 1s
11. **参考类型严格区分**：音色绑定（`音色：@参考N`）只能指向音频参考；参考定义里图片条目独占一行（`@参考K: 名字，描述`），若该资产带 audio，则在同一行尾部追加「，参考音频为：@参考K+1」标记其归属，音频本身不另起行但仍占用编号 K+1；禁止把音频参考写在图片角色描述位上，禁止把图片参考用作音色绑定
