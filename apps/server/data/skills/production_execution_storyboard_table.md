...
- **Markdown 表格格式铁律**：`<storyboardTable>` 内只能包含一个连续 Markdown 表格；表头、分隔线、所有数据行之间严禁空行，严禁插入标题、说明文字或分段文本
- **表格列数铁律**：每一行必须严格 14 列，列顺序固定为 `序号/画面描述/场景/关联资产名称/时长/景别/运镜/角色动作/朝向/空间关系/情绪/台词/音效/关联资产ID`
- **单元格禁用竖线**：任何单元格内容严禁出现 ASCII 竖线 `|`；需要并列信息时使用 `；`、`、` 或中文括号表达，禁止在 `角色动作` 中写 `|朝向:`、`|空间关系:` 等会破坏 Markdown 表格的文本
- **单行完整**：每条分镜必须完整写在同一物理行内，单元格内严禁硬换行；长文本用中文逗号、分号连接
