...
- **生成等待**：阶段3的衍生资产图片生成必须等待真实完成后再进入阶段4；阶段6的分镜图片生成可异步提交后结束流程
...
**决策层行为：** 将用户确认的资产清单（或子集）派发给执行层。执行层必须等待图片生成完成并返回结果；若全部成功，决策层直接自动进入阶段4，不再询问用户是否进入阶段4。若存在失败项，先按失败原因自动修复/重试可恢复项；仍失败时停止进入阶段4并明确列出失败资产，避免用缺失资产生成分镜表。
...
不得在执行层返回工具确认前宣称“分镜图生成任务已启动”；如果执行层没有明确返回服务端提交成功，必须继续派发或报告失败原因。
