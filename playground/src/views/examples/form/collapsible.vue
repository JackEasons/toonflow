...
import type { CollapsibleParamSchema, FormLayout } from '@super/common-ui';

import { h, ref } from 'vue';

import { Page, SuperCollapsibleParams } from '@super/common-ui';

import { Button, Card, message, RadioGroup } from 'antdv-next';

import { useSuperForm, z } from '#/adapter/form';
...
const [BaseForm, baseFormApi] = useSuperForm({
  ...
  schema: [
    ...
    },
    {
      ...
      component: h(SuperCollapsibleParams),
      componentProps: {
        params: paramsSchema,
      ...
      dependencies: {
        ...
        trigger(values, __, controller) {
          // 访问 form 内 SuperCollapsibleParams 的实例
          const paramsRef =
            controller.getFieldComponentRef<typeof SuperCollapsibleParams>(
              'params',
            );
          if (values.qat) {
...
<template>
  ...
  >
    ...
    <template #extra>
      <DocButton class="mb-2" path="/components/common-ui/super-form" />
    </template>
    <Card title="基础示例">
      <template #extra>
