...
const [BaseModal, baseModalApi] = useSuperModal({
  // 连接抽离的组件
  connectedComponent: BaseDemo,
});

const [InContentModal, inContentModalApi] = useSuperModal({
  // 连接抽离的组件
  connectedComponent: InContentModalDemo,
});

const [AutoHeightModal, autoHeightModalApi] = useSuperModal({
  connectedComponent: AutoHeightDemo,
});

const [DragModal, dragModalApi] = useSuperModal({
  connectedComponent: DragDemo,
});

const [DynamicModal, dynamicModalApi] = useSuperModal({
  connectedComponent: DynamicDemo,
});

const [SharedDataModal, sharedModalApi] = useSuperModal({
  connectedComponent: SharedDataDemo,
});

const [FormModal, formModalApi] = useSuperModal({
  connectedComponent: FormModalDemo,
});

const [NestedModal, nestedModalApi] = useSuperModal({
  connectedComponent: NestedDemo,
});

const [BlurModal, blurModalApi] = useSuperModal({
...
<template>
  ...
  >
    <template #extra>
      <DocButton path="/components/common-ui/super-modal" />
    </template>
    <BaseModal />
    <InContentModal />
    <AutoHeightModal />
    ...
    <Flex wrap="wrap" class="w-full" gap="10">
      ...
      <Card class="w-75" title="轻量提示弹窗">
        <template #extra>
          <DocButton path="/components/common-ui/super-alert" />
        </template>
