...
**输出内容规则**：
- 选择模式A时：仅输出 `[Prompt]` 正文，并在尾部用 `【负面约束】` 写入模型可理解的规避项；这是提示词正文的一部分，不作为独立 API 参数
...
【负面约束】{风格专属负向词模板；至少包含低清、模糊、畸形肢体、错乱手指、重复人物、字幕、水印、logo、UI界面、标题叠字。}
...
- 模式B 负向词按风格专属「负向词模板」输出到 `"negative"` 字段，每条必须包含，不可省略；模式A 用 `【负面约束】` 段承载同类规避项，不使用独立 API 字段
